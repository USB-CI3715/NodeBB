...
import winston from 'winston'; // Importa la biblioteca 'winston' para el manejo de logs y errores
...
interface IMessaging {
  ...
  pushUnreadCount(uids: string[], unreadData: any): void;
...
module.exports = function (Messaging: IMessaging) {
...
async function sendNotification(Messaging: IMessaging, fromUid: number, roomId: string, messageObj: MessageObj): Promise<void> {
    // Realiza tres operaciones en paralelo:
    // 1. Obtiene la configuración de notificaciones de la sala.
    // 2. Obtiene los datos de la sala.
    // 3. Obtiene los usuarios en tiempo real conectados a la sala.
    ...
    await batch.processSortedSet(`chat:room:${roomId}:uids:online`, async (uids: string[]) => {
